...
from utils.load_CIFAR10_data import build_cifar10_dataloaders
...
class VGG16_BN_CIFAR10_Better(nn.Module):
    def __init__(self, num_classes=10, dropout=0.3):
        ...
        self.features = nn.Sequential(OrderedDict([
            ...
            # Block 4: 4 -> 2
            # =========================
            ("block4_conv1", nn.Conv2d(256, 512, kernel_size=3, padding=1, bias=False)),
            ("block4_bn1", nn.BatchNorm2d(512)),
            ("block4_relu1", nn.ReLU(inplace=True)),

            ("block4_conv2", nn.Conv2d(512, 512, kernel_size=3, padding=1, bias=False)),
            ("block4_bn2", nn.BatchNorm2d(512)),
            ("block4_relu2", nn.ReLU(inplace=True)),

            ("block4_conv3", nn.Conv2d(512, 512, kernel_size=3, padding=1, bias=False)),
            ("block4_bn3", nn.BatchNorm2d(512)),
            ("block4_relu3", nn.ReLU(inplace=True)),

            ("block4_pool", nn.MaxPool2d(kernel_size=2, stride=2)),

            # =========================
            # Block 5: 維持 2x2，不再做 maxpool 到 1x1
            # =========================
            ("block5_conv1", nn.Conv2d(512, 512, kernel_size=3, padding=1, bias=False)),
            ("block5_bn1", nn.BatchNorm2d(512)),
            ("block5_relu1", nn.ReLU(inplace=True)),

            ("block5_conv2", nn.Conv2d(512, 512, kernel_size=3, padding=1, bias=False)),
            ("block5_bn2", nn.BatchNorm2d(512)),
            ("block5_relu2", nn.ReLU(inplace=True)),

            ("block5_conv3", nn.Conv2d(512, 512, kernel_size=3, padding=1, bias=False)),
            ("block5_bn3", nn.BatchNorm2d(512)),
...
if __name__ == "__main__":
    ...
    # 輸出路徑
    OUTPUT_ROOT = "outputs/my_vgg16_experiment/run2"
